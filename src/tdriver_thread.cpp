...
RubyThread::RubyThread( QObject *parent ) : QThread( parent ) {
...
bool RubyThread::start() {
    ...
    if ( !QFile::exists( listener ) && !listenerNotFound ) {
        ...
        msgBoxContent = 
            QString( "Could not find Visualizer listener server:\n" ) + 
            QString( listener ) + 
            QString( "\n\nFile must be located in same directory as Visualizer executable.  " );

        msgBoxContent = 
            QString( "Could not start Visualizer listener server.\n\n" ) + 
            QString( "Server file (listener.rb) must be located in same directory as Visualizer executable.  " );
...
//                            QString( resultByteArray.data() );
    ...
    if ( !execute_status ) {
        ...
        // mention something of going to offline...
